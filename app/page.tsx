...
import { Package, Boxes, Warehouse, Inbox, ArrowUpFromLine, Clock, ChevronRight, TrendingUp } from 'lucide-react';
...
export default function Home() {
  ...
  const kpis = isAdmin ? [
    { label: 'Productos', sub: 'tipos distintos', value: (stats as AdminStats)?.totalProductos, iconClass: 'kpi-icon-blue', border: 'border-t-blue-500', icon: Package },
    { label: 'Unidades', sub: 'en inventario', value: (stats as AdminStats)?.totalInventario, iconClass: 'kpi-icon-green', border: 'border-t-emerald-500', icon: Boxes },
    { label: 'Almacenes', sub: 'registrados', value: (stats as AdminStats)?.almacenes, iconClass: 'kpi-icon-amber', border: 'border-t-amber-500', icon: Warehouse },
    { label: 'Pendientes', sub: 'por aprobar', value: (stats as AdminStats)?.movimientosPendientes, iconClass: 'kpi-icon-red', border: 'border-t-rose-500', icon: Inbox },
  ] : isEncargado ? [
    { label: 'Mis Almacenes', sub: 'asignados', value: (stats as any)?.almacenesCount, iconClass: 'kpi-icon-amber', border: 'border-t-amber-500', icon: Warehouse },
    { label: 'Pendientes', sub: 'por aprobar', value: (stats as any)?.movimientosPendientes, iconClass: 'kpi-icon-red', border: 'border-t-rose-500', icon: Inbox },
    { label: 'En stock', sub: 'unidades', value: (stats as any)?.productosEnStock, iconClass: 'kpi-icon-green', border: 'border-t-emerald-500', icon: Boxes },
    { label: 'Enviadas', sub: 'esperando respuesta', value: (stats as any)?.pendingSent, iconClass: 'kpi-icon-blue', border: 'border-t-blue-500', icon: ArrowUpFromLine },
  ] : [
    { label: 'Recepciones', sub: 'pendientes', value: (stats as OperatorStats)?.pendingReceptions, iconClass: 'kpi-icon-red', border: 'border-t-rose-500', icon: Inbox },
    { label: 'En stock', sub: 'unidades', value: (stats as OperatorStats)?.productosEnStock, iconClass: 'kpi-icon-green', border: 'border-t-emerald-500', icon: Boxes },
    { label: 'Enviadas', sub: 'esperando respuesta', value: (stats as OperatorStats)?.pendingSent, iconClass: 'kpi-icon-blue', border: 'border-t-blue-500', icon: ArrowUpFromLine },
  ];
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <div className="main-content">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 lg:pt-8 space-y-6">
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold lg:text-3xl text-gray-900">
                {(() => {
                  ...
                  const nombre = (session?.user as any)?.nombre || (session?.user as any)?.name || '';
                  return nombre ? `${saludo}, ${nombre.split(' ')[0]}` : saludo;
                })()}
              </h1>
              <p className="text-sm mt-1" style={{ color: 'var(--text-3)' }}>
                {isAdmin ? 'Control global del sistema de inventarios' : almacenNombre ? `Almacén · ${almacenNombre}` : 'Stock disponible y movimientos de tu almacén'}
              </p>
            </div>
            {!loading && (
              <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-100 flex-shrink-0 mt-1">
          ...
          {/* KPI Cards */}
          <div className={`grid gap-3 ${isAdmin || isEncargado
            ? 'grid-cols-2 lg:grid-cols-4'
            : 'grid-cols-1 sm:grid-cols-3'
            }`}>
            {loading
              ? Array.from({ length: isAdmin || isEncargado ? 4 : 3 }).map((_, i) => (
                <div key={i} className="card-stat p-4 border-t-4 border-t-gray-200">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-xl bg-gray-200 animate-pulse flex-shrink-0" />
              ...
              : kpis.map(({ label, sub, value, iconClass, border, icon: Icon }) => (
                <div key={label} className={`card-stat p-4 border-t-4 ${border}`}>
                  <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${iconClass} shadow-sm flex-shrink-0`}>
                      <Icon className="h-5 w-5" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-2xl font-bold leading-none tracking-tight font-sans">
                        {(value ?? 0).toLocaleString()}
                      </p>
                      <p className="text-[10px] mt-1 font-medium text-muted-foreground truncate">
